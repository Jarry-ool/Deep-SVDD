...
class VisualizationManager:
    ...
    def _set_font(self, lang: str = 'cn'):
        """根据语言设置字体"""
        if lang == 'cn':
    ...
    def plot_zerone_preview(self, images: List[np.ndarray], labels: List[int], 
                            lang: str = 'cn'):
        """Zerone分支专属样本预览 - 支持图像和1D向量两种模式"""
        n = min(len(images), 8)
        
        # 检查是否为1D向量（MLP模式）
        sample = images[0]
        is_1d = sample.ndim == 1 or (sample.ndim == 2 and sample.shape[0] == 1)
        
        if is_1d:
            # MLP模式：绘制特征向量的折线图
            fig, axes = plt.subplots(2, 4, figsize=(14, 6))
            title = 'Zerone特征向量预览 (MLP模式)' if lang == 'cn' else 'Zerone Feature Vector Preview (MLP Mode)'
            fig.suptitle(title, fontsize=14, fontweight='bold')
            
            for i in range(n):
                ax = axes[i // 4, i % 4]
                feat = images[i].flatten()
                
                # 分段显示：时域(0-15), STFT(15-142), PSD(142-342), 其他
                ax.plot(feat[:50], 'b-', alpha=0.7, linewidth=0.5)  # 只显示前50维
                
                label_text = '正常' if labels[i] == 0 else '故障'
                if lang == 'en':
                    label_text = 'Normal' if labels[i] == 0 else 'Fault'
                ax.set_title(f"{label_text} (前50维)", fontsize=9,
                            color=COLORS['normal'] if labels[i] == 0 else COLORS['fault'])
                ax.set_xlabel('维度' if lang == 'cn' else 'Dim', fontsize=8)
                ax.grid(True, alpha=0.3)
    ...
    def plot_svdd_feature_space(self, features: np.ndarray, scores: np.ndarray, 
                                  center: np.ndarray = None, lang: str = 'cn'):
        """SVDD特征空间可视化 (t-SNE降维 + 异常得分着色)"""
        from sklearn.manifold import TSNE
        
        L = {
            'cn': {'title': 'SVDD特征空间', 'cbar': '异常得分', 'center': '超球中心'},
            'en': {'title': 'SVDD Feature Space', 'cbar': 'Anomaly Score', 'center': 'Hypersphere Center'}
        }[lang]
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        scatter = ax.scatter(features_2d[:, 0], features_2d[:, 1], c=scores, 
                            cmap='RdYlGn_r', alpha=0.6, s=20)
        plt.colorbar(scatter, ax=ax, label=L['cbar'])
        
        ax.set_title(L['title'], fontsize=14, fontweight='bold')
        ax.set_xlabel('t-SNE 1')
        ax.set_ylabel('t-SNE 2')
        
        plt.tight_layout()
        save_path = self.subdirs["stage1"] / f"svdd_feature_space_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] SVDD特征空间已保存: {save_path}")
    
    def plot_error_analysis(self, error_info: List[Dict], all_preds: List[int], 
                            all_labels: List[int], all_probs: List[float], lang: str = 'cn'):
        """错误溯源分析 - 包含错误样本展示、置信度分布、错误类型统计"""
        L = {
            'cn': {
                'title': '错误分类分析',
                'conf_title': '错误样本置信度分布',
                'type_title': '错误类型统计',
                'sample_title': '典型错误样本',
                'fn': '漏报(FN)\n故障→正常',
                'fp': '误报(FP)\n正常→故障',
            ...
            'en': {
                ...
                'conf_title': 'Error Sample Confidence Distribution',
                'type_title': 'Error Type Statistics',
                'sample_title': 'Typical Error Samples',
                'fn': 'False Negative\nFault→Normal',
                'fp': 'False Positive\nNormal→Fault',
        ...
        # 计算错误类型
        fn_count = sum(1 for p, l in zip(all_preds, all_labels) if l == 1 and p == 0)  # 漏报
        fp_count = sum(1 for p, l in zip(all_preds, all_labels) if l == 0 and p == 1)  # 误报
        
        # 错误样本的置信度
        fn_probs = [all_probs[i] for i in range(len(all_preds)) if all_labels[i] == 1 and all_preds[i] == 0]
        fp_probs = [all_probs[i] for i in range(len(all_preds)) if all_labels[i] == 0 and all_preds[i] == 1]
        
        # 使用3行布局: 第1行统计图(2个), 第2-3行错误样本(8个)
        fig = plt.figure(figsize=(14, 12))
        
        # 子图1: 错误类型统计 (第1行左)
        ax1 = fig.add_subplot(3, 2, 1)
        bars = ax1.bar([L['fn'], L['fp']], [fn_count, fp_count], color=['#e74c3c', '#3498db'])
        ax1.set_title(L['type_title'], fontsize=12, fontweight='bold')
        ax1.set_ylabel(L['count'])
        for bar, val in zip(bars, [fn_count, fp_count]):
        ...
        if fn_probs:
            ax2.hist(fn_probs, bins=20, alpha=0.7, label=L['fn'].replace('\n', ' '), color='#e74c3c')
        if fp_probs:
            ax2.hist(fp_probs, bins=20, alpha=0.7, label=L['fp'].replace('\n', ' '), color='#3498db')
        ax2.set_title(L['conf_title'], fontsize=12, fontweight='bold')
        ax2.set_xlabel(L['conf'])
        ax2.set_ylabel(L['count'])
        ax2.legend()
        
        # 子图3-10: 典型错误样本 (第2-3行, 每行4个)
        n_show = min(8, len(error_info))
        for i in range(n_show):
            # 第2行: 位置3-6 (索引0-3), 第3行: 位置7-10 (索引4-7)
            row = 1 + i // 4  # 第2行或第3行
            col = i % 4       # 第0-3列
            ...
            # 检查是否为1D向量（zerone模式）
            if img.ndim == 1 or (img.ndim == 2 and min(img.shape) == 1):
                # 1D特征向量：用折线图显示
                feat = img.flatten()[:100]  # 只显示前100维
                ax.plot(feat, 'b-', linewidth=0.8)
                ...
                if img.ndim == 3 and img.shape[0] == 3:
                    img = np.transpose(img, (1, 2, 0))
                img = np.clip((img - np.nanmin(img)) / (np.nanmax(img) - np.nanmin(img) + 1e-8), 0, 1)
                ax.imshow(img)
                ax.axis('off')
            
            true_l = '正常' if info.get('true', 0) == 0 else '故障'
            pred_l = '正常' if info.get('pred', 0) == 0 else '故障'
            if lang == 'en':
                true_l = 'N' if info.get('true', 0) == 0 else 'F'
                pred_l = 'N' if info.get('pred', 0) == 0 else 'F'
            ax.set_title(f'T:{true_l} P:{pred_l}', fontsize=9, color='red')
        
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["misclassified"] / f"error_analysis_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] 错误分析已保存: {save_path}")
    
    def plot_feature_separability(self, features: np.ndarray, labels: np.ndarray, lang: str = 'cn'):
        """潜在特征空间可分性分析 - 类内/类间距离、Fisher准则"""
        L = {
            'cn': {
                'title': '特征空间可分性分析',
                'dist_title': '类内/类间距离',
                'fisher_title': 'Fisher判别比',
                'intra': '类内距离',
                'inter': '类间距离',
                'normal': '正常类',
                'fault': '故障类',
            ...
            'en': {
                'title': 'Feature Space Separability Analysis',
                'dist_title': 'Intra/Inter Class Distance',
                'fisher_title': 'Fisher Discriminant Ratio',
                'intra': 'Intra-class',
                'inter': 'Inter-class',
                'normal': 'Normal',
                'fault': 'Fault',
        ...
        fault_feats = features[labels == 1]
        
        # 计算类中心
        normal_center = normal_feats.mean(axis=0)
        fault_center = fault_feats.mean(axis=0)
        
        # 类内距离 (到类中心的平均距离)
        intra_normal = np.mean(np.linalg.norm(normal_feats - normal_center, axis=1))
        intra_fault = np.mean(np.linalg.norm(fault_feats - fault_center, axis=1))
        
        # 类间距离
        inter_dist = np.linalg.norm(normal_center - fault_center)
        
        # Fisher判别比 (每维)
        normal_var = normal_feats.var(axis=0) + 1e-8
        fault_var = fault_feats.var(axis=0) + 1e-8
        fisher_ratio = (normal_center - fault_center) ** 2 / (normal_var + fault_var)
        ...
        ax = axes[0]
        x = [L['intra'] + '\n' + L['normal'], L['intra'] + '\n' + L['fault'], L['inter']]
        vals = [intra_normal, intra_fault, inter_dist]
        colors = ['#2ecc71', '#e74c3c', '#3498db']
        bars = ax.bar(x, vals, color=colors)
        ax.set_title(L['dist_title'], fontsize=12, fontweight='bold')
        ...
        # 子图2: Fisher比值直方图
        ax = axes[1]
        ax.hist(fisher_ratio, bins=50, color='#9b59b6', alpha=0.7)
        ax.axvline(fisher_ratio.mean(), color='red', linestyle='--', label=f'Mean={fisher_ratio.mean():.2f}')
        ax.set_title(L['fisher_title'], fontsize=12, fontweight='bold')
        ax.set_xlabel(L['ratio'])
        ax.legend()
        
        # 子图3: Top-10 Fisher比值特征
        ax = axes[2]
        top_k = 10
        top_indices = np.argsort(fisher_ratio)[-top_k:][::-1]
        ax.barh(range(top_k), fisher_ratio[top_indices], color='#e67e22')
        ax.set_yticks(range(top_k))
        ax.set_yticklabels([f'Dim {i}' for i in top_indices])
        ax.set_xlabel(L['ratio'])
        ax.set_title(f'Top-{top_k} {L["fisher_title"]}', fontsize=12, fontweight='bold')
        ax.invert_yaxis()
        
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["stage3"] / f"feature_separability_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] 特征可分性分析已保存: {save_path}")
    
    def plot_zerone_stft_stats(self, zerone_feats: np.ndarray, labels: np.ndarray, lang: str = 'cn'):
        """Zerone的STFT段均值统计"""
        L = {
            'cn': {
                'title': 'STFT段均值统计 (正常 vs 故障)',
                'xlabel': '频段索引',
                'ylabel': '均值',
                'normal': '正常',
                'fault': '故障'
            ...
            'en': {
                'title': 'STFT Segment Mean Statistics (Normal vs Fault)',
        ...
        # STFT特征在15-142维 (127维)
        stft_feats = zerone_feats[:, 15:142]
        normal_stft = stft_feats[labels == 0]
        fault_stft = stft_feats[labels == 1]
        ...
        ax = axes[0]
        x = np.arange(127)
        ax.plot(x, normal_stft.mean(axis=0), 'g-', label=L['normal'], linewidth=1.5)
        ax.fill_between(x, normal_stft.mean(axis=0) - normal_stft.std(axis=0),
                       normal_stft.mean(axis=0) + normal_stft.std(axis=0), alpha=0.2, color='green')
        ax.plot(x, fault_stft.mean(axis=0), 'r-', label=L['fault'], linewidth=1.5)
        ax.fill_between(x, fault_stft.mean(axis=0) - fault_stft.std(axis=0),
                       fault_stft.mean(axis=0) + fault_stft.std(axis=0), alpha=0.2, color='red')
        ...
        # 差异热力图
        ax = axes[1]
        diff = fault_stft.mean(axis=0) - normal_stft.mean(axis=0)
        im = ax.imshow(diff.reshape(1, -1), aspect='auto', cmap='RdBu_r')
        ax.set_xlabel(L['xlabel'])
        ax.set_yticks([])
        plt.colorbar(im, ax=ax, label='Fault - Normal')
        
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["feature_preview"] / f"stft_stats_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] STFT统计已保存: {save_path}")
    
    def plot_time_domain_correlation(self, zerone_feats: np.ndarray, labels: np.ndarray, 
                                      split_name: str = 'val', lang: str = 'cn'):
        """时域特征相关性图"""
        L = {
            'cn': {'title': f'时域特征相关性矩阵 ({split_name})', 'names': [
                '均值', '标准差', '最大值', '最小值', '峰峰值', '峰值因子', '裕度因子',
                '偏度', '峰度', '均方根', '波形因子', '脉冲因子', '方差', '绝对均值', '四次方'
            ]},
            'en': {'title': f'Time Domain Feature Correlation ({split_name})', 'names': [
                'Mean', 'Std', 'Max', 'Min', 'PP', 'Crest', 'Margin',
                'Skew', 'Kurt', 'RMS', 'Shape', 'Impulse', 'Var', 'AbsMean', '4thMom'
            ]}
        }[lang]
        ...
        # 时域特征在0-15维
        time_feats = zerone_feats[:, :15]
        corr = np.corrcoef(time_feats.T)
        
        fig, ax = plt.subplots(figsize=(10, 8))
        import seaborn as sns
        sns.heatmap(corr, annot=True, fmt='.2f', cmap='RdBu_r', center=0, ax=ax,
                   xticklabels=L['names'], yticklabels=L['names'], annot_kws={'size': 7})
        ax.set_title(L['title'], fontsize=12, fontweight='bold')
        
        plt.tight_layout()
        save_path = self.subdirs["feature_preview"] / f"time_correlation_{split_name}_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] 时域相关性已保存: {save_path}")
    
    def plot_psd_waterfall(self, zerone_feats: np.ndarray, labels: np.ndarray,
                           split_name: str = 'val', lang: str = 'cn'):
        """PSD瀑布图 (按类别分组)"""
        L = {
            'cn': {'title': f'PSD瀑布图 ({split_name})', 'normal': '正常样本', 'fault': '故障样本',
                  'freq': '频率 (Hz)', 'sample': '样本索引', 'psd': 'PSD'},
            'en': {'title': f'PSD Waterfall ({split_name})', 'normal': 'Normal', 'fault': 'Fault',
                  'freq': 'Frequency (Hz)', 'sample': 'Sample Index', 'psd': 'PSD'}
        }[lang]
        
        self._set_font(lang)
        
        # PSD特征在142-342维 (200维, 对应1-200Hz)
        psd_feats = zerone_feats[:, 142:342]
        normal_psd = psd_feats[labels == 0][:50]  # 取前50个样本
        fault_psd = psd_feats[labels == 1][:50]
        
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        
        # 正常样本瀑布图
        ax = axes[0]
        im = ax.imshow(normal_psd, aspect='auto', cmap='viridis', origin='lower')
        ax.set_title(L['normal'], fontsize=12, fontweight='bold')
        ax.set_xlabel(L['freq'])
        ax.set_ylabel(L['sample'])
        plt.colorbar(im, ax=ax, label=L['psd'])
        
        # 故障样本瀑布图
        ax = axes[1]
        im = ax.imshow(fault_psd, aspect='auto', cmap='viridis', origin='lower')
        ax.set_title(L['fault'], fontsize=12, fontweight='bold')
        ax.set_xlabel(L['freq'])
        ax.set_ylabel(L['sample'])
        plt.colorbar(im, ax=ax, label=L['psd'])
        ...
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["feature_preview"] / f"psd_waterfall_{split_name}_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] PSD瀑布图已保存: {save_path}")
    
    def plot_dual_branch_analysis(self, hetero_feats: np.ndarray, zerone_feats: np.ndarray,
                                   labels: np.ndarray, lang: str = 'cn'):
        """双分支融合样本分析"""
        from sklearn.manifold import TSNE
        ...
        L = {
            ...
            'en': {
                'title': 'Dual Branch Feature Space Comparison',
                'hetero': 'Hetero Branch',
                'zerone': 'Zerone Branch',
        ...
        # 对每个分支做t-SNE
        for idx, (feats, title) in enumerate([
            (hetero_feats, L['hetero']),
            (zerone_feats, L['zerone']),
            (np.concatenate([hetero_feats, zerone_feats], axis=1), L['fused'])
        ]):
            ax = axes[idx]
            if feats.shape[1] > 2:
                tsne = TSNE(n_components=2, random_state=42, perplexity=min(30, len(feats)-1))
                feats_2d = tsne.fit_transform(feats[:500])  # 限制样本数加速
                labels_sub = labels[:500]
            else:
                feats_2d = feats[:500]
                labels_sub = labels[:500]
            
            for label, color, name in [(0, 'green', L['normal']), (1, 'red', L['fault'])]:
                mask = labels_sub == label
                ax.scatter(feats_2d[mask, 0], feats_2d[mask, 1], c=color, label=name, alpha=0.5, s=20)
        ...
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["stage3"] / f"dual_branch_analysis_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] 双分支分析已保存: {save_path}")
    
    def plot_gate_distribution(self, gate_weights: np.ndarray, labels: np.ndarray, lang: str = 'cn'):
        """门控权重分布 + 交叉门控散点图"""
        L = {
            'cn': {
                'title': '门控权重分析',
                'dist_title': '门控权重分布',
                'scatter_title': '交叉门控散点图',
                'hetero_w': 'Hetero权重',
                'zerone_w': 'Zerone权重',
                'normal': '正常',
                'fault': '故障'
            ...
            'en': {
                'title': 'Gate Weight Analysis',
                'dist_title': 'Gate Weight Distribution',
                'scatter_title': 'Cross Gate Scatter',
                'hetero_w': 'Hetero Weight',
                'zerone_w': 'Zerone Weight',
                'normal': 'Normal',
                'fault': 'Fault'
            }
        }[lang]
        ...
        # 门控权重分布直方图
        ax = axes[0]
        if gate_weights.shape[1] >= 2:
            ax.hist(gate_weights[:, 0], bins=30, alpha=0.7, label=L['hetero_w'], color='#3498db')
            ax.hist(gate_weights[:, 1], bins=30, alpha=0.7, label=L['zerone_w'], color='#e74c3c')
        ax.set_title(L['dist_title'], fontsize=12, fontweight='bold')
        ax.legend()
        ...
        if gate_weights.shape[1] >= 2:
            for label, color, name in [(0, 'green', L['normal']), (1, 'red', L['fault'])]:
                mask = labels == label
                ax.scatter(gate_weights[mask, 0], gate_weights[mask, 1], 
                          c=color, label=name, alpha=0.5, s=20)
        ax.set_xlabel(L['hetero_w'])
        ax.set_ylabel(L['zerone_w'])
        ax.set_title(L['scatter_title'], fontsize=12, fontweight='bold')
        ax.legend()
        ax.plot([0, 1], [1, 0], 'k--', alpha=0.3)  # 对角线
        
        plt.suptitle(L['title'], fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_path = self.subdirs["stage3"] / f"gate_distribution_{lang}.png"
        fig.savefig(save_path, dpi=self.viz_dpi, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print(f"[可视化] 门控分布已保存: {save_path}")
