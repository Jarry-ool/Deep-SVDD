...
class AnomalyModelV5(nn.Module):
    ...
    def __init__(self, branch_mode: str = 'dual', fusion_mode: str = 'gmu',
        ...
        # SVDD投影头（简化结构，降低容量防止崩塌）
        # 原: 512->256->ReLU->Dropout->64 (容易过拟合到center)
        # 现: 512->128->BN->ReLU->64 (更浅，更少参数)
        self.svdd_proj = nn.Sequential(
            nn.Linear(512, 128),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.1),  # LeakyReLU保持梯度流动
    ...
    def init_center(self, dataloader: DataLoader, device: torch.device, gpu_transform=None):
        """初始化SVDD中心"""
        n = 0
        ...
        self.eval()
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="初始化SVDD中心", leave=False, ncols=80):
                data, zr, _, _ = batch
