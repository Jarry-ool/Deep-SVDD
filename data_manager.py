...
class DataSplitManager:
    ...
    def load_from_filter_output(self, filter_output_dir: Path) -> bool:
        ...
        支持两种格式:
        - v1: validation_samples.json, test_samples.json
        - v2: val/正常/, val/故障/, test/正常/, test/故障/ 目录结构
        """
        ...
        # 尝试v1格式 (json文件)
        val_json = filter_dir / "validation_samples.json"
        test_json = filter_dir / "test_samples.json"
        ...
        if val_json.exists() and test_json.exists():
            ...
            print(f"[DataSplitManager] v1格式: 加载 {len(self.val_samples)} 验证 + {len(self.test_samples)} 测试")
            return True
        
        # 尝试v2格式 (目录结构)
        val_dir = filter_dir / "val"
        test_dir = filter_dir / "test"
        
        if val_dir.exists() or test_dir.exists():
            print("[DataSplitManager] 检测到v2格式目录结构")
            
            # 扫描val目录
            if val_dir.exists():
                self.val_samples = self._scan_labeled_dir(val_dir)
                print(f"  验证集: {len(self.val_samples)} 样本")
            
            # 扫描test目录
            if test_dir.exists():
                self.test_samples = self._scan_labeled_dir(test_dir)
                print(f"  测试集: {len(self.test_samples)} 样本")
            
            return len(self.val_samples) > 0 or len(self.test_samples) > 0
        
        print(f"[警告] 未找到有效的划分数据: {filter_dir}")
        return False
    
    def _scan_labeled_dir(self, data_dir: Path) -> List[Dict]:
        """
        扫描标注数据目录，返回样本列表 (并行优化)
        
        目录结构: data_dir/正常/*.jsonl, data_dir/故障/*.jsonl
        """
        ...
        def _process_file(jsonl_file):
            ...
            return {
                'file': str(jsonl_file.absolute()),
                'label': label,
            }
        
        # 并行处理
        with ThreadPoolExecutor(max_workers=16) as executor:
            samples = list(executor.map(_process_file, all_files))
    ...
    def scan_train_data(self, csv_files: List[Path], signal_len: int = 8192,
        ...
        for csv_path in tqdm(csv_files, desc="扫描训练数据", ncols=80, leave=True):
